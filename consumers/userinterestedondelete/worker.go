package userinterestedondelete
...
func worker(topic string, partition int, offset int64, headers []kafka.Header, key, value []byte) (string, error) {
	...
	if userMessage.Meta.Action == "DELETE" {
		fmt.Printf("proccesing %v DELETE ACTION with user id: %v, partition: %v, offset: %v\n", topic, userMessage.UserId, partition, offset)
		time.Sleep(500 * time.Millisecond) // simulate 500ms latency proccess
		fmt.Printf("processed %v DELETE ACTION with user id: %v, partition: %v, offset: %v\n", topic, userMessage.UserId, partition, offset)
		return "SUCCESS", nil

	} else {
		fmt.Printf("skip %v non DELETE ACTION with user id: %v, partition: %v, offset: %v\n", topic, userMessage.UserId, partition, offset)
		return "SKIP", nil
	}
}
...
func NewSuperVisor() *supervisor.EventWorker {
	config := kafka.ReaderConfig{
		...
		GroupID:     "consumer-user-interested-on-delete-simple-supervisor-v1",
		Topic:       "user-event-v1",
		MinBytes:    5,
		MaxBytes:    10e6,
